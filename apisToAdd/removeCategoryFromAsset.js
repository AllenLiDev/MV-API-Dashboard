...
const foobar = (i) => {
    for (let count = (0 + i); count < (1000 + i); count++) {
        deleteCategory(catArray[count])
    }
    console.log("iteration over")
    setTimeout(() => {
        console.log("waiting ------- 60 seconds -----------------------------------------------------------------------------------------------------------------------------------------");
        console.log(i + " / total " + catArray.length);
        if (i < ids.length) {
            foobar(i);
        }
    }, 60000);
...
const deleteCategory = (id) => {
    var options = {
        'method': 'DELETE',
        'url': 'https://mv-api-usca.mediavalet.com/assets/' + id + '/categories/fe451970-a8b7-4851-97bf-8d89a6912160',
        'headers': {
